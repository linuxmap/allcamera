...
{
    // 存放密钥的缓冲区
    unsigned char encryptBuf[256] =
    {
        0
    };

    //进行加密
    if (NULL == EncryptPass16(pass, encryptBuf))
    {
        return NULL;
    }

    unsigned char * p = encryptBuf;

    // 存放加密后字符串的缓冲区
    char    localPassword[256] = {0};

    // 将密文转换成字符串形式
    char    *pPassword = localPassword;
    p = encryptBuf;
    for (int j = 0; j < 16; j++)
    {
        sprintf(pPassword, "%02x", *p++);
        pPassword += 2;
    }
    memcpy(output, localPassword, 32);
    return output;
}
