...
int32_t CSVS_Mb_Buffer_Manager::init()
{
    ...
    enum _MB_TYPE_
    {
        MB_TYPE_ALARM_EVENT = 0, //告警事件消息类型
        MB_TYPE_NORMAL_EVENT,    //其它信令消息类型
        MB_TYPE_CALL_BACK,       //回调消息类型
        MB_TYPE_SEND_MSG,        //发送消息类型
        MB_TYPE_EVENT_MSG,      // Rt内部事件消息类型
        MB_TYPE_MAX_VALUE
    };
    ...
    uint32_t buffCount[MB_TYPE_MAX_VALUE][MB_LEN_MAX_VALUE] =
        {
            { 25000, 0, 0,0, 0 },     //告警事件消息类型
            { 5000, 20000, 4000, 160, 50 }, //其它信令消息类型
            { 40000, 0, 0, 5000,0 },     //回调消息类型
            { 20000, 4000, 1000, 0,50 },   //发送消息类型
            {3000, 0, 0, 0, 0}         // Rt内部事件消息类型
        };
                                                        ...
                                                        uint32_t mbSize ) const
{
    switch ( mbType )
    {
    case MB_TYPE_ALARM_EVENT:   //告警事件消息
    ...
    case MB_TYPE_CALL_BACK:     //回调消息
        return get_mb_buffer_call_back(mbSize);
    case MB_TYPE_SEND_MSG:    //发送消息类型
        return get_mb_buffer_send_msg(mbSize);
    case MB_TYPE_EVENT_MSG: // Rt内部事件消息类型
        if (mbSize <= SVS_128_BYTE)
        {
